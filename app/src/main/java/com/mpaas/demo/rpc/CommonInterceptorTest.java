...
public class CommonInterceptorTest implements RpcInterceptor {
    ...
    public boolean preHandle(Object o, ThreadLocal<Object> threadLocal, byte[] bytes, Class<?> aClass, Method method, Object[] objects, Annotation annotation, ThreadLocal<Map<String, Object>> threadLocal1) throws RpcException {
        ...
        if (object instanceof GetIdGetReq) {
            GetIdGetReq req = (GetIdGetReq) object;
            req.isMale = false;
        } else if (object instanceof PostPostReq) {
            PostPostReq req = (PostPostReq) object;
        ...
        if ("com.antcloud.request.exception".equals(e.getOperationType())) {
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    Toast.makeText(context, context.getString(R.string.rpc_exception_toast, e.getCode()), Toast.LENGTH_SHORT).show();
                }
            });
            return false;
        }
